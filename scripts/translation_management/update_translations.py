...
def main():
    ...
    run_command(extract_cmd, "🔍 Extraction des textes translatable")
    ...
    for lang in languages:
        ...
        if po_file.exists():
            ...
            run_command(update_cmd, f"🔄 Mise à jour des traductions {lang.upper()}")
        else:
            # Initialize new language
            init_cmd = f'uv run pybabel init -d "{translations_dir}" -i "{translations_dir}/template.pot" -l {lang} -D {domain}'
            run_command(init_cmd, f"✨ Initialisation de la langue {lang.upper()}")

    # Step 3: Compile
    compile_cmd = f'uv run pybabel compile -d "{translations_dir}" -D writing_assistant'
    run_command(compile_cmd, "⚙️  Compilation des traductions (.po → .mo)")
